...
- `activate()` поднимает окно, переводит в него фокус и подсвечивает рамку. Опция `highlightOnActivate: false` отключает подсветку; по умолчанию она включена. Настройка доступна также через `update(options)` и декларативный `options`. `bringToFront()` меняет только порядок.
