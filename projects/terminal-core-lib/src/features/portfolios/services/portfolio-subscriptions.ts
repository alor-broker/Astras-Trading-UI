...
import {PortfolioItemsModelHelper} from '../utils/portfolio-items-model.helper';
...
export class PortfolioSubscriptionsService {
  ...
  getAllPositionsSubscription(portfolio: string, exchange: string): Observable<Position[]> {
    ...
    return this.getOrCreateSubscription(
      ...
      request => of(new Map<string, Position>()).pipe(
        mapWith(
          () => this.subscriptionsDataFeedService.subscribe<PortfolioRequestBase, PositionWarpResponse>(request, this.getSubscriptionKey),
  ...
  private getCurrentSessionOrders(portfolio: string, exchange: string): Observable<Order[] | null> {
    return this.httpClient.get<OrderResponse[]>(`${this.baseUrl}/${exchange}/${portfolio}/orders`)
      .pipe(
  ...
  private getCurrentSessionStopOrders(portfolio: string, exchange: string): Observable<StopOrder[] | null> {
    return this.httpClient.get<StopOrderResponse[]>(`${this.baseUrl}/${exchange}/${portfolio}/stoporders`)
      .pipe(
  ...
  private getCurrentSessionTrades(portfolio: string, exchange: string): Observable<Trade[] | null> {
    return this.httpClient.get<TradeResponse[]>(`${this.baseUrl}/${exchange}/${portfolio}/trades`, {
      params: {
        format: 'heavy'
      }
    })
