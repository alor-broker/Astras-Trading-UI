...
@Injectable()
export class WsOrdersConnector implements NetworkStatusProvider, OrderDelayProvider, OnDestroy {
  private readonly webSocketOrdersUrlProvider = inject(WEB_SOCKET_ORDERS_URL_PROVIDER);
  ...
  private createWebSocketSubject(socketState: SocketState): WebSocketSubject<WsResponseMessage> {
    ...
    const subject = this.webSocketFactory<WsResponseMessage>({
      url: this.webSocketOrdersUrlProvider.cwsUrl,
      openObserver: {
        next: () => {
