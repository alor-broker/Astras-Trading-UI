...
import {ControlValueAccessorBase} from '../../../features/forms/components/control-value-accessor-base';
import {
  takeUntilDestroyed,
  toObservable
...
export class InputNumber extends ControlValueAccessorBase<number> {
  ...
  private removeExtraSymbol(symbol: string, input: string): string {
    const symbolIndexes = Array.from(input).reduce((previousValue: number[], currentValue, currentIndex) => {
