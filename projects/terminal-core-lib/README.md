...
## Контекст выставления заявки

`SUBMIT_ORDER_CONTEXT` из `features/orders/types/submit-order-context.types.ts` отделяет вызов торгового сценария
от платформенного интерфейса. Метод `submitOrder(params: SubmitOrderParams)` запускает форму заявки,
но не отправляет торговую операцию автоматически.

Модель содержит ключ инструмента, начальные значения формы и необязательную `side`.
Без `side` доступны оба направления; с `side` форма разрешает только переданное направление.
`OrderDialogParams` переиспользует эту модель; прежний импорт `OrderFormType` остаётся совместимым.

Пока provider подключён только в desktop. Общие виджеты используют optional injection и скрывают действие,
если платформа не предоставляет контекст. `ACTIONS_CONTEXT` остаётся ответственным за выбор инструмента.
