...
Структура папки: `components/`, `services/`, `directives/`, `types/` (модели в файлах `*.types.ts`), `utils/` (хелперы в файлах `*.helper.ts`).
...
| `<ats-widget-settings-editor>` (`WidgetSettingsEditor`) | Точка входа в шаблоне settings-компонента; держит состояние и выбирает раскладку | Используй в шаблоне |
...
| `WidgetSettingsLayout{Desktop,Mobile}` | Раскладки desktop/mobile | Не трогай напрямую — выбираются редактором |
| `WidgetSettingsDialogService` | CDK-overlay диалога на desktop | Внутренняя деталь, вызывается редактором |
| `WidgetSettingsEditorRef` (тип) | Интерфейс, по которому `ats-widget-skeleton` получает редактор | Реализован `WidgetSettingsBase`, передаётся в skeleton |
...
Settings-компонент рендерится **рядом** со скелетоном (sibling) и передаётся в него ссылкой:
...
<ats-widget-skeleton
  ...
  [showEditingPlaceholder]="true"
>
  <ng-template #headerRef>
    <ats-widget-header (switchSettings)="settingsCmp.openSettings($event)" [hasSettings]="true" .../>
  </ng-template>
  <ng-template #contentRef> <!-- контент виджета --> </ng-template>
</ats-widget-skeleton>

<ats-x-settings #settingsCmp [guid]="guid" [widgetInstance]="widgetInstance()"/>
```
...
- `[settingsEditor]="settingsCmp"` — передаёт ссылку (`WidgetSettingsEditorRef`); skeleton сам определяет, что показать (inline-редактор → placeholder → контент).
- `[showEditingPlaceholder]="true"` — **опционально**. Включи для виджетов, которые не применяют настройки «на лету»: пока открыт редактор, контент заменяется на заглушку «Редактирование настроек» и пересоздаётся при закрытии. По умолчанию выключено.
- Убери из skeleton legacy-инпуты `[settings]` / `[showSettings]`.
...
Редактор device-agnostic: внутри него один `@if (isMobile())`, который делегирует всю отрисовку в `WidgetSettingsLayoutDesktop` / `WidgetSettingsLayoutMobile`. Различия:

| | Desktop | Mobile |
| --- | --- | --- |
| Контейнер | Модальный диалог (CDK overlay) у шестерёнки | Inline в слоте контента виджета (шапка виджета видна) |
...
- [ ] В шаблоне виджета: settings-компонент — sibling skeleton с ref; `(switchSettings)` → `openSettings($event)`; legacy `[settings]`/`[showSettings]` убраны.
- [ ] Модели — в `*.types.ts`, без `export function` (используй классы-хелперы).
- [ ] Переводы новых меток добавлены в `ru`/`en`/`hy`.
- [ ] Стили компонента не нужны для общих `.one-row` рядов — они уже в `widget-settings-editor.less`.
