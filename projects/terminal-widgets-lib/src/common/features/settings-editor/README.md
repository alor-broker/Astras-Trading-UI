...
Для компактных виджетов без отдельного заголовка можно включить `[allowContentDrag]="true"` у `WidgetSkeleton`. По умолчанию опция выключена: содержимое блокирует начало перетаскивания. Область настроек блокирует его всегда; кнопки и другие интерактивные элементы перетаскиваемого содержимого должны самостоятельно останавливать `mousedown`. Пример — `ribbon`.
