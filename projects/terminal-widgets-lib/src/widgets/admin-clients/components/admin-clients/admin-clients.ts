...
import {
  ...
  ClientsSearchFilter,
  SpectraExtension
} from "../../services/admin-clients-service.types";
import {
  BaseTableComponent,
  Sort
...
import {AdminClientRestrictions} from '@terminal-widgets-lib/widgets/admin-clients/components/admin-client-restrictions/admin-client-restrictions';
...
@Component({
  ...
  imports: [
    ...
    NzMenuItemComponent,
    NzTableModule,
    AdminClientRestrictions
  ],
  templateUrl: './admin-clients.html',
  styleUrl: './admin-clients.less',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None
})
export class AdminClients extends BaseTableComponent<ClientDisplay, ClientsSearchFilter> implements OnInit, OnDestroy {
  ...
  private settings$!: Observable<AdminClientsWidgetSettings>;
  ...
  override ngOnInit(): void {
    this.settings$ = this.settingsService.getSettings<AdminClientsWidgetSettings>(this.guid()).pipe(
  ...
  override saveColumnWidth(event: { columnId: string, width: number }): void {
    super.saveColumnWidth<AdminClientsWidgetSettings>(event, this.settings$);
  }
    ...
    AdminDashboardsHelper.openDashboardForPortfolio(
      {
        ...
        marketType: PortfolioKeyHelper.getMarketTypeByPortfolio(client.portfolio)
      },
