...
@Component({
  ...
  templateUrl: './import-watchlist-dialog.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ImportWatchlistDialog implements OnInit {
  inputString = '';
    ...
    this.marketService.getDefaultExchange().pipe(
      ...
      const itemsToResolve = tickers.map(t => {
        ...
        return this.instrumentsService.getInstrument(instrumentKey).pipe(
          ...
          map(i => ({
            input: rawTicker,
            instrumentKey: i ? InstrumentKeyHelper.toInstrumentKey(i) : null
          } as ParsedItem))
        );
      })
