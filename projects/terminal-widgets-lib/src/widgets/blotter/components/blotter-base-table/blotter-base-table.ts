...
import {ExportHelper} from '@terminal-widgets-lib/widgets/blotter/utils/export-helper';
...
export abstract class BlotterBaseTable<T extends { id: string }, F extends object>
  ...
  override ngOnInit(): void {
    this.settings$ = this.settingsService.getSettings<BlotterWidgetSettings>(this.guid())
      .pipe(
        shareReplay(1),
  ...
  override saveColumnWidth(event: { columnId: string, width: number }): void {
    super.saveColumnWidth<BlotterWidgetSettings>(event, this.settings$);
  }

  openContextMenu($event: MouseEvent, menu: AddToWatchlistMenu, selectedRow: T): void {
