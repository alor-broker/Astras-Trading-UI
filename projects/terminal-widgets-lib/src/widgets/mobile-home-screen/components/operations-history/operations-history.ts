...
@Component({
  ...
  styleUrls: ['./operations-history.less'],
...
export class OperationsHistory implements OnInit {
  ...
  private bindAgreement(): void {
    combineLatest([
      ...
      map(([selectedKey, allPortfolios]) => allPortfolios.find(p => PortfolioKeyEqualityComparer.equals(p, selectedKey))?.agreement ?? null),
      distinctUntilChanged(),
  ...
  private createRequestParams(offset: number): HistoryFilterParams {
    ...
    return {
      ...
      dateFrom: dateFrom != null ? formatISO(dateFrom, {representation: 'date'}) : undefined,
      dateTo: dateTo != null ? formatISO(dateTo, {representation: 'date'}) : undefined
    };
  }
