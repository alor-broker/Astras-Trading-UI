...
import {ScalperOrderBook} from '@terminal-widgets-lib/widgets/scalper-order-book/components/scalper-order-book/scalper-order-book';
...
@Component({
  ...
  templateUrl: './scalper-order-book-widget.html',
  styleUrls: ['./scalper-order-book-widget.less'],
  imports: [
    TranslocoDirective,
    WidgetSkeleton,
    WidgetHeader,
...
export class ScalperOrderBookWidget extends WidgetBase<ScalperOrderBookWidgetSettings> {
  protected dashboardContextService = inject(DASHBOARD_CONTEXT_SERVICE);

  protected override createSettingsIfMissing(): void {
    WidgetSettingsFactoryHelper.createInstrumentLinkedWidgetSettingsIfMissing<ScalperOrderBookWidgetSettings>(
      this.widgetInstance(),
      'ScalperOrderBookSettings',
      settings => ({
        ...settings,
        ...
        showWorkingVolumesPanel: ValueHelper.getValueOrDefault(settings.showInstrumentPriceDayChange, true),
