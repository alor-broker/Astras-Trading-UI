...
} from "../types/scalper-order-book-data-context.types";
...
export interface DataContextBuilderDeps {
  ...
  readonly instrumentTradesService: InstrumentTradesService;
}
...
export class DataContextBuilder {
  ...
  private static mapToBodyRows(
    ...
    const isOrderbookInstrumentCorrect = InstrumentEqualityComparer.equals(settings.widgetSettings, orderBook.instrumentKey);
    if (!InstrumentEqualityComparer.equals(settings.widgetSettings, rowsState.instrumentKey)
    ...
    const currentInstrumentLocalOrders$ = settings$.pipe(
      ...
      map(x => {
        return Array.from(x.localOrders.values())
          .filter(o => {
            return InstrumentEqualityComparer.equals(o.targetInstrument, x.s.widgetSettings)
  ...
  private static getInstrumentTradesStream(
    ...
    let results: InstrumentTradesItem[] = [];

    const getTrades = (instrumentKey: InstrumentKey): Observable<InstrumentTradesItem | null> => deps.instrumentTradesService.getNewTradesSubscription(instrumentKey, 100).pipe(
      startWith(null)
    );
