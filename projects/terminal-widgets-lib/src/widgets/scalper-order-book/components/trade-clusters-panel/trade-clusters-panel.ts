...
import {TradesCluster} from '@terminal-widgets-lib/widgets/scalper-order-book/components/trades-cluster/trades-cluster';
...
@Component({
  ...
  templateUrl: './trade-clusters-panel.html',
  styleUrls: ['./trade-clusters-panel.less'],
  imports: [
    TranslocoDirective,
...
export class TradeClustersPanel implements OnInit, OnDestroy, AfterViewInit {
  ...
  clusters$!: Observable<TradesClusterType[]>;
  ...
  trackBy(index: number, item: TradesClusterType): number {
    return item.timestamp;
  }

  contextMenu($event: MouseEvent, menu: NzDropdownMenuComponent): void {
    menu.nzOverlayClassName = 'ats-trade-clusters-panel';
