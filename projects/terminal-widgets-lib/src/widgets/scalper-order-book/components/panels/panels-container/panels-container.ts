...
@Component({
  ...
  templateUrl: './panels-container.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  providers: [
    {
      provide: PANELS_CONTAINER_CONTEXT,
      useExisting: PanelsContainer
    }
  ]
})
export class PanelsContainer implements PanelsContainerContext, AfterViewInit, OnDestroy {
  readonly panelsQuery = contentChildren(Panel);
  ...
  private readonly panels = new ReplaySubject<Panel[]>(1);
  ...
  public expandPanel(panel: Panel): void {
    this.panels.pipe(
      take(1)
    ).subscribe(panels => {
  ...
  private normalizeSavedWidths(rawWidths: Record<string, number>, panels: Panel[]): Map<string, number> {
  ...
  private recalculateWidths(allPanels: Panel[], resizedPanel: Panel, event: ResizedEvent): Map<string, number> | null {
