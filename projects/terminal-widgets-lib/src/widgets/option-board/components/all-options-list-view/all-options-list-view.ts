...
@Component({
  ...
  templateUrl: './all-options-list-view.html',
  styleUrl: './all-options-list-view.less',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None
})
export class AllOptionsListView implements OnInit, OnDestroy {
  ...
  private initCurrentPrice(): void {
    this.currentPrice$ = this.dataContext().settings$.pipe(
      switchMap(settings => {
        return this.quotesService.getQuotesSubscription(settings.symbol, settings.exchange).pipe(
          map(quote => {
            if (quote == null) {
              return null;
            }
