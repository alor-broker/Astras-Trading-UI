...
import {OptionPreview} from '@terminal-widgets-lib/widgets/option-board/components/option-preview/option-preview';
...
@Component({
  ...
  templateUrl: './all-options.html',
  styleUrls: ['./all-options.less'],
  imports: [
    TranslocoDirective,
    NzSpinComponent,
    NzEmptyComponent,
...
export class AllOptions implements OnInit, AfterViewInit, OnDestroy {
  ...
  getDaysToExpirations(expirationDate: Date): number {
    return differenceInCalendarDays(expirationDate, new Date());
  }
