...
import {
  ChartParameters,
  YieldCurveChartParameters
} from "../yield-curve-chart-parameters/yield-curve-chart-parameters";
import {
  DurationType,
  YieldType
...
@Component({
  ...
  styleUrls: ['./yield-curve-chart.less'],
  imports: [
    TranslocoDirective,
...
export class YieldCurveChart implements OnInit, OnDestroy {
  ...
  private settings$!: Observable<BondScreenerWidgetSettings>;
  ...
  ngOnInit(): void {
    this.settings$ = this.widgetSettingsService.getSettings<BondScreenerWidgetSettings>(this.guid())
      .pipe(shareReplay(1));
