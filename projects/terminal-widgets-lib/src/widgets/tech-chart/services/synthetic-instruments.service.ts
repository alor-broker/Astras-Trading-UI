...
} from '@terminal-widgets-lib/widgets/tech-chart/types/synthetic-instruments.model';
import {
  Candle,
  HistoryResponse
      ...
      .pipe(
        ...
        map(histories => {
                ...
                if (!historyCopy.map(sh => sh.time).includes(h.time)) {
                  ...
                  if (nextCandleIndex !== 0) {
                    historyCopy.splice(
                      ...
                      0,
                      {
                        ...historyCopy[(nextCandleIndex === -1 ? historyCopy.length : nextCandleIndex) - 1],
                        time: h.time
                      }
                    );
                  }
          ...
          return histories.map(h => {
            ...
            return {
              isSpreadOperator: false,
              value: instrumentsHistories[historyIndex++]
            } as InstrumentDataPart<HistoryResponse>;
          });
        }),
