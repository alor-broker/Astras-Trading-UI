...
- Не создавай отдельные переводы только для `aria-label` и других атрибутов доступности.
  Используй перевод видимой подписи или тултипа. Для кнопок с иконкой добавляй тултип:
  его текст и `aria-label` должны совпадать, включая параметры и текущее состояние действия.
  В шаблоне вычисляй общую подпись через `@let` и используй её в обоих bindings.
...
- Тултипы и `aria-label` одного действия используют одинаковый перевод и параметры;
  нет отдельных ключей только для атрибутов доступности.
