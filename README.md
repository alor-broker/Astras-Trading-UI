...
Терминал использует следующие инструменты.
 ...
 - ngx-device-info - для получении информации об устройстве, на котором запущено приложение
...
### Environment настройки
 ...
 - logging: настройки логгирования (опциональные)
   ...
   - remote: настройки логов, записываемых в elastic (опциональные - если данная секция отсутствует, то логи в elastic не записываются)
     - minLevel: минимальный уровень записываемых логов (доступные значения в порядке повышения уровня: 'trace', 'info', 'warn', 'error')
     ...
     - authorization: настройки авторизации elastic (заполняются CI/CD)
